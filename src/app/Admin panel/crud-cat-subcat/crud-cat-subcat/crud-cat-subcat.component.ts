...
export class CrudCatSubcatComponent implements OnInit {
  ...
  onADDCat(form: NgForm) {
    if (!form.valid) {
      ...
      console.log(c);
      this.adminserv.createcat(c).subscribe(
        (res) => {
  ...
  onADDSubcat(form: NgForm) {
    if (!form.valid) {
      ...
      console.log(a);
      this.adminserv.createsubcat(a).subscribe(
        (res) => {
          this.summary = "Success";
  ...
  onDeleteCat(form: NgForm) {
    ...
    this.adminserv.deletecat(this.selectedCat2.CategoryID).subscribe(
      (res) => {
        this.summary = "Success";
  ...
  onDeleteSubCat(form: NgForm) {
    this.adminserv.deletesubcat(this.selectedsubCat.SubCategoryID).subscribe(
      (res) => {
        this.summary = "Success";
